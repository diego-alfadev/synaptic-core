...
- **Shared / global brain guidance.** The upgrade procedure (`references/upgrade-to-v1.md`) gains a hardening addendum covering the multi-repo case (one `.synaptic/` serving many sibling repos): explicit per-repo wiring, re-pointed brain pointer, and the Detect-on-Load "stay silent when a bridge is present" rule so a wired repo never offers to create a competing nested brain.
